...
model_log_returns <- function(ticker, start_date, end_date, models_df=NULL) {
  ...
  model_results <- c(name = ticker, AR = arima_model$arma[1], I = arima_model$arma[6], MA = arima_model$arma[2])
...
for (ticker in companies) {
  models_df <- model_log_returns(ticker, "2013-01-01", "2022-12-31", models_df)
  print(i)
  i<-i+1
  print(models_df)
}
...
model_log_returns_v2 <- function(ticker, start_date="2010-01-01",
                                 end_date = "2022-12-31",
                                 models_df=NULL) {
  ...
  # Creamos un vector con los resultados del modelo ajustado
 c(AR = arima_model$arma[1], I = arima_model$arma[6], MA = arima_model$arma[2])
}


models_df <- apply(companies, 2, model_log_returns_v2)
